...
%
{\item {\bf Stage 1:} $ZH$ production is splitted into production via $q\bar{q}$ or $gg$ initial states. Our process sample was generated at LO and only contains $q\bar{q}\to ZH$ events. Events are subsequently classified according to the value of $p_T^Z$ and number of extra jets\footnote{Jets are reconstructed using the Anti-$K_t$ clustering algorithm with radius parameter $R=0.4$. Jets are further demanded to have $p_T^j>20$ GeV.} in the event as follows:
%
\vspace{0.25cm}
%
\begin{center}
...
\end{itemize}
%
As explained above,% in order to reject the $Zb\bar{b}$ background, 
in the selection of the events that populate each of the STXS bins we have also used the first BDT discriminant to separate $Zb\bar{b}$ from SM $ZH$ production. 
This is simply introduced to optimize the sensitivity of the measurements to $ZH$, as in the actual experimental seaches. 
No extra information or discriminant to enhance sensitivity to new physics is used, consistently with the STXS hypotheses. 
The different STXS bin cross sections obtained after the categorization are shown in Figure~\ref{fig:stxs_crosssec} for each of the generated samples.
...
Total cross section contained in each of the STXS bins for each of the generated samples.
}
\end{figure}
